...
Let $\mathsf{V}$ be a vector space having dimension $n$, and let $\set{S}$
be a subset of $\mathsf{V}$ that generates $\mathsf{V}$.

(a) Prove that there is a subset of $\set{S}$ that is a basis for $\mathsf{V}$.
(Be careful not to assume that $\set{S}$ is finite.)

\begin{quotation}
    \textbf{Theorem 1.9.} If a vector space $\mathsf{V}$ is generated by a finite set $\set{S}$, then
    some subset of $\set{S}$ is a basis for $\mathsf{V}$. Hence, $\mathsf{V}$
    has a finite basis.
\end{quotation}
